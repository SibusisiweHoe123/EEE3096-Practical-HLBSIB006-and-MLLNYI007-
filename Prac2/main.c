...
static uint16_t storedAddress = 0;
...
int main(void)
{
  ...
  uint16_t eepromAdd = 0;
    for (int i = 0; i < 6; i++){
  	  write_to_address(eepromAdd, patterns[eepromAdd]);
  	  eepromAdd++;
    }
...
void TIM16_IRQHandler(void)
{
	...
	// Read the binary value from EEPROM
		uint8_t binValue = read_from_address(storedAddress);

	if (binValue == patterns[storedAddress]){
		// Display binary value on LEDs
	     GPIOB -> ODR |= binValue;
	  }
	  else {
		  GPIOB -> ODR |= 1;  // You might need to adapt this mapping
	  }
